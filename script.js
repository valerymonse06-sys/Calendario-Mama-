...
function iniciarSeguridad() {
    const pinGuardado = localStorage.getItem("app_pin");
    
    if (!pinGuardado) {
        // Primera vez: Crear contraseña
        const nuevoPin = prompt("¡Bienvenida! ✨\nCrea un PIN de acceso para proteger tu calendario:");
        if (nuevoPin && nuevoPin.length >= 4) {
            localStorage.setItem("app_pin", btoa(nuevoPin)); // Encriptación simple
        ...
        // Siguientes veces: Pedir contraseña
        const login = prompt("Introduce tu PIN para ver el calendario:");
        if (btoa(login) !== pinGuardado) {
            alert("PIN incorrecto ❌");
            document.body.innerHTML = `
                <div style="display:flex; justify-content:center; align-items:center; height:100vh; background:#F5F3EF; font-family:serif;">
                    <h2 style="color:#1a3a52;">Acceso denegado. Recarga la página para intentar de nuevo.</h2>
                </div>`;
...
// Ejecutar seguridad inmediatamente
iniciarSeguridad();
